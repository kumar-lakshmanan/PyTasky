...
class PTSFlowRunner(QtCore.QThread):
    ...
    def preSetup(self):
        ...
        for each in flowData["nodes"]:
            ...
            self.nodes[name]['tags'] = nodeModule.TAGS if hasattr(nodeModule, 'TAGS') else []
            ...
            self.nodes[name]['actualop'] = nodeModule.OUTPUTS if hasattr(nodeModule, 'OUTPUTS') else self._getDefaultOutputs(self.nodes[name]['tags'])
            ...
            if not self._isItSystemNodeForNode(self.nodes[name]) and self._getNodeStyle2(self.nodes[name]) == "STARTER": self.currentExecutionNodes.append(self.nodes[name])
        ...
        while len(self.currentExecutionNodes) and not self.terminateFlowExecution:
            ...
            #If its Sys Node
                ...
                if self._isTagPresentInTags("condition", currentNode['tags']):
                    ...
                    conditionStr = currentNode['props']['condition'].strip()
                    conditionStatus = self.tls.console.runCommand(conditionStr)
                    if conditionStatus:
                        ...
                        if not nextNode in self.currentExecutionNodes: self.currentExecutionNodes.append(nextNode)
                ...
                if self._isTagPresentInTags("loop", currentNode['tags']):
                    ipPort = 'inpList'
                    opPort = self.tls.cfg['pts']['defaultOutPortName']
                    ...
                    if type(inputForNode[ipPort]) == type([]):
                        ...
                        lst = self.tls.console.runCommand(inputForNode[ipPort])
                    ...
                    if lst and type(lst)==type([]):
                        ...
                        if loopCoreNode in self.currentExecutionNodes: self.currentExecutionNodes.remove(loopCoreNode)
                ...
                self.terminateFlow("Unknown node. Node should be either system or custom. But this is not valid: {currentNode}")
    ...
    def updateExecutionLocalsWithNeededInput(self, inputForNode):
        ...
        self.tls.console.updateLocals('data', self.nodesOutputData)
                       
        #Each inp with its values.
        for eachKey in inputForNode:
            self.tls.console.updateLocals(eachKey, inputForNode[eachKey])    
