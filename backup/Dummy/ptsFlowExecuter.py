...
class FlowExecuter(QtCore.QThread):  # Inherit QThread
    finished_signal = QtCore.pyqtSignal()  # Signal to notify when execution stops

    def __init__(self, parent):
        super().__init__()
        self.parent = parent

    def run(self):  # This function runs in a separate thread
        print("Flow execution started...")

        while (len(self.parent.currentProcessing)>0):
            QApplication.processEvents()
            # if self.debugging and self.debugWait:
            #     if nowNode: nowNode.set_selected(False)                
            #     continue
            nowNode = self.parent.currentProcessing.pop()
            nowNode.set_selected(True)
            if not self.parent.isInputReadyForNode(nowNode):
                self.parent.currentProcessing.append(nowNode)
            else:
                inpData = self.parent.getRequestData(nowNode)
                print('-----------')                
                respData = nowNode.nodeAction(inpData)
                print('-----------')
                if self.parent.processResponseData(nowNode, respData):
                    nextNodes = self.parent.getNextNodes(nowNode)
                    for eachNextNodes in nextNodes:
                        if not eachNextNodes in self.parent.currentProcessing: self.parent.currentProcessing.insert(0,eachNextNodes)                        
                else:
                    self.parent.tls.error(f"Response invalid for {inpData} : {respData}")
                    break;
...
class PTSFlowExecuter(object):
    ...
    def __init__(self, parent):
        ...
        # self.debugging = 0
        # self.debugWait = 0
        #self.flowRunner = ptsFlowRunner.PTSFlowRunner(self)
        #self.flowRunner.runFlow()
...
# """
        ...
        self.console.runCode(dataToExecute)
        mr = self.console.ptsConsole.locals["mres"]
        print(f"My RES:  {mr}")
